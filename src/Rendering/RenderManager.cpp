    ...
    m_lightBufferDirty(false)
{
	...
	for (uint i = 0; i < 2; i++)
	{
		...
		glTexStorage2D(GL_TEXTURE_2D, levels, GL_RGB16F, m_bloomSize, m_bloomSize);
		setTextureParams(GL_TEXTURE_2D, GL_CLAMP_TO_EDGE, GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR, levels);
	}
...
void RenderManager::render()
{
	...
	// Read from the 4th mip level only
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 3);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 3);
	
	// Blur X
	setViewportSize(64, 64);
	perFrame.invScreenSize = 1.0f / glm::vec2(64);
	m_perFrameBuffer->updateAll(&perFrame);
	setRenderState(RENDER_STATE::COLOR);
	
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, m_bloomFBOs[1]);
	glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_bloomTextures[1], 3);
	...
	glBindTexture(GL_TEXTURE_2D, m_bloomTextures[1]);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, m_bloomFBOs[0]);
	//glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_bloomTextures[0], 3);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 3);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 3);
